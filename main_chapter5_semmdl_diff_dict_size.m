...
% dict_size_per_classes    = [2:2:16]; % Yale_B
...
for dataset=[2]
    ...
    if dataset == 1
        dict_size_per_classes    = [2:2:16]; % Yale_B
    elseif dataset == 2
        dict_size_per_classes    = [1:7];
    end
    
    reco_rates= [];
    
    for f = dict_size_per_classes
        
        fprintf('\n\nf = %d\n',f);
        
        if (dataset == 1)
            load YaleB_DR_DAT
            num_atom_per_class = f; % f
            dname = 'Extended Yale B dataset';
            
        elseif (dataset == 2)
            load AR_DR_DAT
            num_atom_per_class = f;
            dname = 'AR dataset';
